...
def calculate_random_paths(graph, departureTime, k, startNode): #!!! OPENING HOURS NOT IMPLEMENTED !!!~
    ...
    for i in range(k):
        paths[i].append((startNode, utils.format_time(departureTime)))
    ...
    while nodesToVisit:
        for i in range(k):
            ...
            if not nodesToVisit:
                for j in range(k):
                    if currentNodes[j] != startNode:
                        arrivalTime[j] += graph.nodes[currentNodes[j]]['inspectionDuration'] + utils.time_to_seconds(graph.edges[currentNodes[j],startNode]['travelTime']) #add last inspection and travel time to startNode
                        paths[j].append((startNode, utils.format_time(arrivalTime[j])))  # append startNode and final arrival time for each vehicle
                return paths
            ...
            travel_time = utils.time_to_seconds(graph.edges[currentNode,nextNode]['travelTime'])
            ...
            vehiclePath.append((nextNode, utils.format_time(arrivalTime[i])))
